...
noiseLvl = noiseLvls{3};

simulationRound(h_true, M, K, 'bpspn3', f_filter, f_filter2, ...
    xGauss, xGauss, xTwoTone, noiseLvl, fs)
...
%     x_predist = applyVolterra(x, h_cur, M, 1:K);
%     h_rls_online_g{ii} = rlsOnlineSimulation(x_predist, M, 1:K, @(x, xi) f_filter2(t, x, xi), @addNoise, h_cur);
%     
%     % RLA
%     h_cur = h_rla_g{ii-1};
%     x_predist = applyVolterra(x, h_cur, M, 1:K);
...
%     x_predist = applyVolterra(x, h_cur, M, 1:K);
%     h_rls_online_mln{ii} = rlsOnlineSimulation(x_predist, M, 1:K, @(x, xi) f_filter2(t, x, xi), @addNoise, h_cur);
%     
%     % RLA
...
%     x_predist = applyVolterra(x, h_cur, M, 1:K);
%     h_rls_online_two_tone{ii} = rlsOnlineSimulation(x_predist, M, 1:K, @(x, xi) f_filter2(t, x, xi), @addNoise, h_cur);
%     
%     % RLA
...
function result = addNoise(x, mu, sigma)
    if nargin < 3
        sigma = 0.005/3;
    end
    if nargin < 2
